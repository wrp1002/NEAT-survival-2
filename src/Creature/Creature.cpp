...
void Creature::ApplyGenes(b2World &world, string genes) {
    ...
    for (int i = 0; i < 10; i ++) {
        int size = (i * 10) + 10;
        shared_ptr<BodySegment> newPart = make_shared<BodySegment>(BodySegment(world, shared_from_this(), b2Vec2(size, size / 2), al_map_rgb(100, 100, 100), i % 2, prevPart, Util::DegreesToRadians(15), Util::DegreesToRadians(10)));
